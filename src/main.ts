import { delegate, getURLHash, insertHTML, replaceHTML } from './helpers.ts';
import { TodoStore } from './store.js';
import './index.css';
import { tourMachine } from './tourMachine.ts';
import { createActor } from 'xstate';

const tourActor = createActor(tourMachine);

const Todos = new TodoStore('todo-modern-vanillajs', tourActor);
...
tourActor.subscribe((snapshot) => {
  const value = snapshot.value;

  const descriptionNode = snapshot._nodes.find((node) => !!node.description);

  App.$.tour!.innerHTML = descriptionNode!.description ?? value;
});
